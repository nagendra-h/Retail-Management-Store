...
class PRODUCT:
        ...
        ######## PRODUCT FRAME=============
            ...
            #+++++TITLE+++++++++++++
            titile=Label(pr_frame,text="PRODUCT DETALS",font=("goudy old style",15),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
            ...
            add_p=Button(pr_frame,text="ADD",command=self.add,font=("times new roman" ,15),bg="black",fg="orange",bd=4)
            add_p.place(x=30,y=350,width=110)
            

            delete_p=Button(pr_frame,text="DELETE",command=self.delete,font=("times new roman" ,15),bg="black",fg="orange",bd=4)
            delete_p.place(x=200,y=350,height=40,width=130)

            update_p=Button(pr_frame,text="UPDATE",command=self.update,font=("times new roman" ,15),bg="black",fg="orange",bd=4)
            update_p.place(x=30,y=410,height=40,width=130)


            clear_p=Button(pr_frame,text="CLEAR",command=self.clear,font=("times new roman" ,15),bg="black",fg="orange",bd=4)
            ...
            view_i.place(x=510,y=120,width=600,height=200)


            self.Logo=Image.open("images/cat2.jpg")
            self.Logo=self.Logo.resize((700,230),Image.ANTIALIAS)
            self.Logo=ImageTk.PhotoImage(self.Logo)

            LeftMenuLogo=Label(view_i,image=self.Logo)
            LeftMenuLogo.place(x=0,y=0,relwidth=1,height=190) 
            ...
            t_titile=Label(self.root,text="VIEW",font=("goudy old style",15),bg="#0f4d7d",fg="white").place(x=510,y=330,width=600,height=30)
           
            view_p=Frame(self.root,bd=3,relief=RIDGE)
            view_p.place(x=510,y=370,width=600,height=180)
                ...
                try:
                    ...
                    if self.var_cat.get()=="select" or self.var_item_name.get()=="":
                        messagebox.showerror("Error","Category must be required ",parent=self.root)
                            ...
                            '''
                        cur.execute("Select * from product where  item_no=?",(self.var_item_no.get(),))
                        row=cur.fetchone()
        ...
        def update(self):
                 ...
                 try:
                    if self.var_prod_no.get()=="":
                        ...
                        cur.execute("Select * from product where prod_no=?",(self.var_prod_no.get(),))
                        row=cur.fetchone()
                        if row==None:
                            messagebox.showerror("Error","INVALID  data  ",parent=self.root)
                        else:
                            cur.execute("Update  product set category=?,id=?,item_no=?,item_name=?,price=?,iqty=?,status=?  where  prod_no=?",(
                                self.var_cat.get(),
                                self.var_id.get(),
                                self.var_item_no.get(),
                                self.var_item_name.get(),
                ...
                try:
                    if self.var_item_no.get()=="":
                        ...
                        if row==None:
                            ...
                            cur.execute("delete from product where prod_no=?",(self.var_prod_no.get(),))
                            if op==True:
                                con.commit()
